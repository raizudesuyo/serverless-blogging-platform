...
WORKDIR /src
COPY ["serverless-blogging-platform.csproj", "serverless-blogging-platform/"]
RUN dotnet restore "serverless-blogging-platform/serverless-blogging-platform.csproj"

WORKDIR "/src/serverless-blogging-platform"
COPY . .
RUN dotnet build "serverless-blogging-platform.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "serverless-blogging-platform.csproj" \
...
Execute unit tests
```
    cd "serverless-blogging-platform/test/serverless-blogging-platform.Tests"
    dotnet test
```

Deploy application
```
    cd "serverless-blogging-platform/src/serverless-blogging-platform"
